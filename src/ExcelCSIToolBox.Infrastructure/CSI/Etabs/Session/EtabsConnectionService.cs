    ...
    public class EtabsConnectionService : ICSISapModelConnectionService
    {
        ...
        public OperationResult<IReadOnlyList<CsiSelectedObjectDto>> GetSelectedObjectsFromActiveModel()
        {
            ...
            try
            {
                ...
                if (ret != 0)
                {
                    return OperationResult<IReadOnlyList<CsiSelectedObjectDto>>.Failure("Failed to read selected objects from ETABS.");
                }
            ...
            {
                return OperationResult<IReadOnlyList<CsiSelectedObjectDto>>.Failure("Failed to read selected objects from ETABS: " + ex.Message);
            }
        }
        ...
        public OperationResult AddFramesToGroup(IReadOnlyList<string> frameNames, string groupName)
        {
            ...
            if (groupRet != 0)
            {
                return OperationResult.Failure($"Failed to create or update ETABS group '{groupName}' (return code {groupRet}).");
            }
            ...
            {
                if (string.IsNullOrWhiteSpace(frameName)) continue;
                int ret = sapModel.FrameObj.SetGroupAssign(frameName, groupName, false, ETABSv1.eItemType.Objects);
                if (ret == 0) success++; else failures.Add($"{frameName}: return code {ret}");
            }
        ...
        public OperationResult<ShellUniformLoadSetContextDto> GetShellUniformLoadSetContext()
        {
            ...
            if (!sapModelResult.IsSuccess)
            {
                return OperationResult<ShellUniformLoadSetContextDto>.Failure(sapModelResult.Message);
            }

            return ExcelCSIToolBox.Infrastructure.CSI.Etabs.Loadings.ShellUniformLoadSets.EtabsShellUniformLoadSetTableService.GetContext(sapModelResult.Data);
        }
        ...
        public OperationResult<IReadOnlyList<ShellUniformLoadSetDefinitionDto>> GetShellUniformLoadSetDefinitions()
        {
            ...
            return ExcelCSIToolBox.Infrastructure.CSI.Etabs.Loadings.ShellUniformLoadSets.EtabsShellUniformLoadSetTableService.GetDefinitions(sapModelResult.Data);
        }
        ...
        public OperationResult<ShellUniformLoadSetApplyResultDto> ApplyShellUniformLoadSets(IReadOnlyList<ShellUniformLoadSetDefinitionDto> definitions)
        {
            ...
            if (!sapModelResult.IsSuccess)
            {
                return OperationResult<ShellUniformLoadSetApplyResultDto>.Failure(sapModelResult.Message);
            }

            return ExcelCSIToolBox.Infrastructure.CSI.Etabs.Loadings.ShellUniformLoadSets.EtabsShellUniformLoadSetTableService.Apply(sapModelResult.Data, definitions);
        }
        ...
        private static int EnsureGroupExists(ETABSv1.cSapModel sapModel, string groupName)
        {
            int numberNames = 0;
            string[] names = null;
        ...
        private static string ObjectTypeIdToName(int typeId)
        {
            switch (typeId)
            {
                ...
                default: return "Object(" + typeId.ToString(CultureInfo.InvariantCulture) + ")";
            }
        }
