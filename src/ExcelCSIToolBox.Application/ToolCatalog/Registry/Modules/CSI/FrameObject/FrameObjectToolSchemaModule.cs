    ...
    public sealed class FrameObjectToolSchemaModule : IToolSchemaModule
    {
        public IEnumerable<ToolSchema> GetSchemas()
        {
            return new[]
            {
                ...
                SchemaModuleHelpers.Schema("FrameObject", "FrameObject_AssignDistributedLoad", "AssignLoad", "FrameObject", new[] { "targetRole", "loadPattern", "direction", "value" }, new[] { "frameNames", "frameGroup", "replace", "coordinateSystem", "startDistance", "endDistance" }, new[] { "loads.frame.assign_distributed", "frames.assign_distributed_load", "assign UDL", "assign distributed load", "apply uniform load", "load top chord", "top chord UDL" }, FrameObjectIntentHints.AssignDistributedLoad, FrameObjectParameterRules.AssignDistributedLoadClarification, true),
