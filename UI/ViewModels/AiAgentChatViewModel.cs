...
namespace ExcelCSIToolBoxAddIn.UI.ViewModels
{
    public class AiAgentChatViewModel : ViewModelBase
    {
        ...
        public AiAgentChatViewModel(IAiChatSessionService chatSessionService)
        {
            ...
            _currentModelName = _chatSessionService.ModelName;
        ...
        public string CurrentModelName
        {
            ...
            private set { _currentModelName = string.IsNullOrWhiteSpace(value) ? "Not selected" : value; OnPropertyChanged(); }
        }

        public string Sap2000ConnectionStatus
        {
            get { return _sap2000ConnectionStatus; }
            private set { _sap2000ConnectionStatus = string.IsNullOrWhiteSpace(value) ? "Not attached" : value; OnPropertyChanged(); }
        }

        public string EtabsConnectionStatus
        {
            get { return _etabsConnectionStatus; }
            private set { _etabsConnectionStatus = string.IsNullOrWhiteSpace(value) ? "Not attached" : value; OnPropertyChanged(); }
        }
        ...
        private async void ExecuteSend(object _)
        {
            ...
            try
            {
                ...
                AiAgentResponse response = await SendWithBufferedStreamingAsync(userMessage, thinkingMessage);

                thinkingMessage.IsTemporary = false;
                thinkingMessage.Content = response.AssistantText;
                UpdateToolTrace(response);
                StatusText = "Ready";
            }
            catch (Exception ex)
            {
        ...
        private Task<AiAgentResponse> SendWithBufferedStreamingAsync(
            string userMessage,
            AiAgentChatMessageViewModel thinkingMessage)
        {
            ...
            return _chatSessionService.SendAsync(
                userMessage,
                token =>
                {
        ...
        private void UpdateToolTrace(AiAgentResponse response)
        {
        ...
        {
            CurrentModelName = _chatSessionService.ModelName;
            EtabsConnectionStatus = _chatSessionService.IsEtabsAttached() ? "Attached" : "Not attached";
            Sap2000ConnectionStatus = _chatSessionService.IsSap2000Attached() ? "Attached" : "Not attached";
        }
    }
