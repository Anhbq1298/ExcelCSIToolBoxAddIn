...
namespace ExcelCSIToolBox.AI.Mcp.Tools.CSI.Truss
{
    public sealed class TrussGenerateHoweTool : IMcpTool, IMcpToolMetadata
    {
        ...
        public string Description => "Generates a Howe truss with optional flat, monoslope, or center-peak/gable top chord, chord/web sections, and distributed load assignment. Chords are continuous frame segments; vertical and brace members are released at both ends.";
    ...
    public sealed class TrussGeneratePrattTool : IMcpTool, IMcpToolMetadata
    {
        ...
        public string Title => "Generate Pratt Truss";
        public string Category => "Truss";
        public string SubCategory => "Generation";
        public string Description => "Generates a Pratt truss with optional flat, monoslope, or center-peak/gable top chord, chord/web sections, and distributed load assignment. Chords are continuous frame segments; vertical and brace members are released at both ends.";
        public bool IsReadOnly => false;
        public CsiMethodRiskLevel RiskLevel => CsiMethodRiskLevel.Low;
        public bool RequiresConfirmation => false;
        public bool SupportsDryRun => false;
        ...
        public Task<ToolCallResponse> ExecuteAsync(string argumentsJson, CancellationToken cancellationToken)
        {
            try
            {
                ...
                PrattTrussRequestDto request = JsonConvert.DeserializeObject<PrattTrussRequestDto>(argumentsJson ?? "{}")
                    ?? new PrattTrussRequestDto();
                request.TrussType = "Pratt";
                OperationResult<HoweTrussResultDto> result = _trussService.Generate(serviceResult.Data, request);
                if (!result.IsSuccess)
                {
                    return Task.FromResult(Fail(result.Message));
            ...
            {
                return Task.FromResult(Fail("Pratt truss generation failed: " + ex.Message));
            }
        }
